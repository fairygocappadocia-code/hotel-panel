...
export default function HotelTimelineVIP() {
  ...
  // ODA İÇİNDEN CARİYE İŞLEME SİHRİ
  const [payCariId, setPayCariId] = useState(""); 
  const [payCariTxType, setPayCariTxType] = useState("arti"); // arti (Borçlanma) / eksi (Ödeme Düş)
  ...
  const [viewStartDate, setViewStartDate] = useState(todayStr); // Takvimin başlangıç noktası
  ...
  // Ekranda 24 günlük bir pencere göster (Sonsuz kaydırma)
  const visibleDays = Array.from({ length: 24 }, (_, i) => {
    const d = new Date(viewStartDate);
    d.setDate(d.getDate() + i - 2); // Geçmiş 2 günü de görerek başla
  ...
  // SİHİRLİ ÖDEME ALMA VE CARİYE İŞLEME FONKSİYONU
  const handleReceivePayment = async () => {
    ...
    // 2. Ana Kasaya Gelir olarak işle
    ...
    // 3. EĞER CARİ SEÇİLDİYSE, CARİYE DE İŞLE
    if (payCariId) {
      const targetCari = caris.find(c => c.id === payCariId);
      if (targetCari) {
        const cDesc = `Oda ${formData.roomNo} - ${payCategory} işlemi (${formData.guestName})`;
        const txObjCari = { 
          id: (Date.now() + 2).toString(), date: todayStr, type: payCariTxType, amount: amountNum, currency: payCurrency, category: payCategory, desc: cDesc 
        };
        const updatedCari = { ...targetCari, transactions: [...(targetCari.transactions || []), txObjCari] };
  ...
  const getStatusColor = (status) => {
    if (status === 'checked_in') return "bg-emerald-500 text-white shadow-emerald-500/40 shadow-lg";
    if (status === 'checked_out') return "bg-rose-500 text-white shadow-rose-500/40 shadow-lg";
    return "bg-indigo-500 text-white shadow-indigo-500/40 shadow-lg";
  };
  ...
  const renderRowCells = (roomNo) => {
    ...
    while (i < visibleDays.length) {
      ...
      if (rez) {
        ...
        cells.push(
          <td key={day} colSpan={span} className="border-r border-b border-slate-200 p-[3px] min-w-[50px] relative h-[42px]">
            <div 
              ...
              className={`${getStatusColor(rez.status)} text-[11px] font-bold px-2 h-full w-full rounded-lg flex items-center cursor-pointer overflow-hidden z-10 transition-all duration-300 hover:scale-[1.03] hover:brightness-110`}
            >
        ...
        cells.push(
          <td key={day} onClick={() => handleOpenNewModal(roomNo, day)} className={`border-r border-b border-slate-200 h-[42px] min-w-[50px] cursor-pointer hover:bg-indigo-50/50 transition-colors ${day === todayStr ? 'bg-indigo-50/30' : 'bg-white'}`}></td>
        );
        i++;
      }
    }
      ...
      <div className="flex h-screen w-screen bg-slate-100 items-center justify-center p-4 font-sans text-slate-800">
        <div className="bg-white border border-slate-200 w-full max-w-md p-10 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.05)] flex flex-col items-center animate-in fade-in zoom-in duration-500">
          ...
          <form onSubmit={handleLogin} className="w-full space-y-5">
            ...
            <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-indigo-200 transition-all text-sm mt-2">Giriş Yap</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex h-screen w-screen bg-slate-100 text-slate-800 font-sans overflow-hidden">
      ...
      <aside className="w-72 bg-white border-r border-slate-200 flex flex-col shadow-[10px_0_30px_rgba(0,0,0,0.03)] z-40 transition-all shrink-0">
        <div className="h-24 flex items-center justify-center md:justify-start px-6 border-b border-slate-100 shrink-0">
        ...
        <nav className="py-6 space-y-2 px-4 border-b border-slate-100 shrink-0">
          <button onClick={() => {setActiveTab("timeline"); setSelectedCariId(null);}} className={`w-full flex items-center justify-start px-4 py-3.5 rounded-2xl transition-all duration-300 ${activeTab === 'timeline' ? 'bg-indigo-50 text-indigo-700 font-bold shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-indigo-600 font-medium'}`}>
            <span className="text-xl">📅</span><span className="ml-3 text-sm">Takvim (PMS)</span>
          </button>
          <button onClick={() => {setActiveTab("kasa"); setSelectedCariId(null);}} className={`w-full flex items-center justify-start px-4 py-3.5 rounded-2xl transition-all duration-300 ${activeTab === 'kasa' ? 'bg-emerald-50 text-emerald-700 font-bold shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-emerald-600 font-medium'}`}>
            <span className="text-xl">💰</span><span className="ml-3 text-sm">Finans & Kasa</span>
          </button>
          <button onClick={() => setActiveTab("caris")} className={`w-full flex items-center justify-start px-4 py-3.5 rounded-2xl transition-all duration-300 ${activeTab === 'caris' ? 'bg-sky-50 text-sky-700 font-bold shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-sky-600 font-medium'}`}>
            <span className="text-xl">👥</span><span className="ml-3 text-sm">Cari Rehberi</span>
          </button>
          <button onClick={() => {setActiveTab("hk"); setSelectedCariId(null);}} className={`w-full flex items-center justify-start px-4 py-3.5 rounded-2xl transition-all duration-300 ${activeTab === 'hk' ? 'bg-purple-50 text-purple-700 font-bold shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-purple-600 font-medium'}`}>
            <span className="text-xl">🧹</span><span className="ml-3 text-sm">Kat Hizmetleri (HK)</span>
          </button>
        </nav>

        {/* GÜNLÜK AKIŞ (SOL MENÜ ALTINDA) */}
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 bg-slate-50/50">
          <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-2">Bugünün Akışı</h4>
          
          <div className="bg-white border border-emerald-100 rounded-2xl p-3 shadow-sm mb-4">
            <h5 className="text-xs font-bold text-emerald-600 mb-2 flex justify-between">Giriş Yapacaklar <span className="bg-emerald-100 px-2 rounded-md">{checkInsToday.length}</span></h5>
            <div className="space-y-1.5 max-h-32 overflow-y-auto custom-scrollbar">
              {checkInsToday.length === 0 ? <p className="text-[10px] text-slate-400 italic">Kayıt yok.</p> : checkInsToday.map(r => (
                <div key={r.id} className="text-[11px] font-medium text-slate-700 bg-slate-50 p-1.5 rounded-lg border border-slate-100 flex justify-between">
                  <span>{r.roomNo} - {r.guestName.substring(0, 12)}</span>
                  {r.status === 'checked_in' && <span className="text-emerald-500 font-bold">✓</span>}
                </div>
              ))}
            </div>
          </div>

          <div className="bg-white border border-rose-100 rounded-2xl p-3 shadow-sm mb-4">
            <h5 className="text-xs font-bold text-rose-600 mb-2 flex justify-between">Çıkış Yapacaklar <span className="bg-rose-100 px-2 rounded-md">{checkOutsToday.length}</span></h5>
            <div className="space-y-1.5 max-h-32 overflow-y-auto custom-scrollbar">
              {checkOutsToday.length === 0 ? <p className="text-[10px] text-slate-400 italic">Kayıt yok.</p> : checkOutsToday.map(r => {
                const { remaining } = getCalc(r);
                return (
                  <div key={r.id} className="text-[11px] font-medium text-slate-700 bg-slate-50 p-1.5 rounded-lg border border-slate-100 flex justify-between items-center">
                    <span>{r.roomNo} - {r.guestName.substring(0, 10)}</span>
                    {remaining > 0 ? <span className="text-rose-500 font-bold">{remaining} B</span> : <span className="text-emerald-500 font-bold">OK</span>}
                  </div>
                )
              })}
            </div>
        ...
        <div className="p-4 border-t border-slate-200 bg-white shrink-0">
          <button onClick={handleLogout} className="w-full flex items-center justify-start px-4 py-3 rounded-xl text-rose-500 hover:bg-rose-50 transition-all font-bold text-sm">
            <span className="text-lg">🚪</span><span className="ml-3">Çıkış Yap</span>
          </button>
        </div>
      </aside>
        ...
        <header className="h-20 bg-white/90 backdrop-blur-xl border-b border-slate-200 px-8 flex justify-between items-center z-20 shrink-0">
          ...
          {activeTab === 'timeline' && (
             <div className="flex items-center gap-1 bg-slate-100 border border-slate-200 rounded-xl p-1 shadow-inner">
               <button onClick={() => shiftDate(-7)} className="px-3 py-2 text-slate-500 hover:text-indigo-600 hover:bg-white rounded-lg font-black transition-all">← Geri</button>
               <button onClick={() => setViewStartDate(todayStr)} className="text-xs font-bold text-indigo-700 bg-white shadow-sm px-5 py-2 rounded-lg transition-all border border-slate-200">Bugün</button>
               <button onClick={() => shiftDate(7)} className="px-3 py-2 text-slate-500 hover:text-indigo-600 hover:bg-white rounded-lg font-black transition-all">İleri →</button>
             </div>
          )}
        </header>

        {/* 1. SEKME: TAKVİM (FULL EKRAN, FOOTERSİZ) */}
        {activeTab === "timeline" && (
          <div className="flex-1 overflow-auto relative custom-scrollbar bg-slate-50/30 animate-in fade-in duration-500">
            <table className="w-full text-left border-collapse table-fixed select-none">
              <thead className="bg-slate-100 text-slate-600">
                <tr>
                  <th className="sticky left-0 top-0 bg-white z-30 w-[70px] border-r border-b border-slate-200 text-center shadow-[2px_0_5px_rgba(0,0,0,0.05)]"><span className="text-[10px] font-black tracking-widest text-slate-500">ODA</span></th>
                  {visibleDays.map(dayObj => (
                    <th key={dayObj.dateStr} className={`w-[50px] sticky top-0 bg-slate-50/95 backdrop-blur-sm z-20 border-r border-b border-slate-200 text-center py-2 ${dayObj.dateStr === todayStr ? 'bg-indigo-50 border-b-2 border-b-indigo-400' : ''}`}>
                ...
                {rooms.map(room => {
                  ...
                  return (
                    <tr key={room} className="hover:bg-slate-50 group/row transition-colors">
                      <td className="sticky left-0 bg-white group-hover/row:bg-slate-50 z-20 border-r border-b border-slate-200 text-center text-slate-700 shadow-[2px_0_5px_rgba(0,0,0,0.03)] h-[42px] font-black text-xs">
          ...
          <div className="flex-1 flex flex-col p-8 overflow-y-auto bg-slate-50 animate-in fade-in duration-500">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 max-w-7xl mx-auto w-full">
              <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xl shadow-slate-200/50 hover:-translate-y-1 transition-transform">
                ...
                <div className={`text-4xl font-black tracking-tight ${kasaBalances.TL.total >= 0 ? 'text-slate-800' : 'text-rose-500'}`}>{kasaBalances.TL.total.toLocaleString()} <span className="text-xl text-slate-400">TL</span></div>
              </div>
              <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xl shadow-slate-200/50 hover:-translate-y-1 transition-transform">
                <h3 className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-2">Dolar Kasa</h3>
                <div className={`text-4xl font-black tracking-tight ${kasaBalances.USD.total >= 0 ? 'text-slate-800' : 'text-rose-500'}`}>{kasaBalances.USD.total.toLocaleString()} <span className="text-xl text-slate-400">USD</span></div>
              </div>
              <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xl shadow-slate-200/50 hover:-translate-y-1 transition-transform">
                <h3 className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-2">Euro Kasa</h3>
                <div className={`text-4xl font-black tracking-tight ${kasaBalances.EUR.total >= 0 ? 'text-slate-800' : 'text-rose-500'}`}>{kasaBalances.EUR.total.toLocaleString()} <span className="text-xl text-slate-400">EUR</span></div>
              </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-lg shadow-slate-200/50 h-fit">
                ...
                <div className="space-y-4">
                  ...
                  <select value={kasaCategory} onChange={e=>setKasaCategory(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm text-slate-700 outline-none focus:border-indigo-500 font-medium">
                    {activities.map(a => <option key={a} value={a}>{a}</option>)}
                    <option value="Market / Mutfak Gideri">Market / Mutfak Gideri</option><option value="Genel Gider">Genel Gider / Diğer</option>
                  </select>

                  <div>
                    <label className="block text-[10px] text-indigo-500 font-bold mb-1">İlgili Cari (Seçersen Cari Bakiyesine de İşler)</label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <input type="number" value={kasaAmount} onChange={e=>setKasaAmount(e.target.value)} placeholder="Tutar" className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm text-slate-800 outline-none" />
                  ...
                  <input type="text" value={kasaDesc} onChange={e=>setKasaDesc(e.target.value)} placeholder="Açıklama (Örn: Yakıt)" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm text-slate-800 outline-none" />
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex justify-center items-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white border border-slate-100 w-full max-w-5xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] animate-in zoom-in-95 duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-8 bg-white flex flex-col md:flex-row gap-8 custom-scrollbar">
              ...
              {/* SAĞ: HESAP & TAHSİLAT & CARİ BAĞLANTISI */}
              <div className="w-full md:w-5/12 bg-slate-50 border border-slate-100 rounded-3xl p-6 flex flex-col shadow-inner">
                ...
                {/* TAHSİLAT & CARİ BAĞLANTISI */}
                <div className="border-t border-slate-200 pt-4 mt-auto">
                  <h4 className="text-xs font-black text-emerald-600 uppercase mb-3">Ödeme Al & Kasa/Cariye İşle</h4>
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      {/* ODA İÇİNDEN CARİ SEÇİMİ */}
                      <select value={payCariId} onChange={e=>setPayCariId(e.target.value)} className="w-full bg-indigo-50 border border-indigo-200 rounded-xl p-3 text-xs text-indigo-700 font-bold outline-none">
                        <option value="">-- Cari İşleme (Yok) --</option>
                        {caris.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                      </select>
                    </div>
                    ...
                    {payCariId && (
                       <div className="flex gap-2">
                         <button onClick={() => setPayCariTxType("arti")} className={`flex-1 py-2 rounded-lg text-[10px] font-bold transition-all border ${payCariTxType === 'arti' ? 'bg-indigo-100 text-indigo-700 border-indigo-300' : 'bg-white text-slate-500 border-slate-200'}`}>Cariye Borç Yaz (+)</button>
                         <button onClick={() => setPayCariTxType("eksi")} className={`flex-1 py-2 rounded-lg text-[10px] font-bold transition-all border ${payCariTxType === 'eksi' ? 'bg-emerald-100 text-emerald-700 border-emerald-300' : 'bg-white text-slate-500 border-slate-200'}`}>Cariden Düş (-)</button>
                       </div>
                    )}
